...
仓库根目录的 `slay-the-spire-standalone.html` 是本项目的**单文件 HTML 版本**，无需构建、双击即玩（含全部美术与音乐素材，约 19MB）。

## 联机合作（仿杀戮尖塔 2）

支持双人合作攀登尖塔：共享地图与敌人，各自拥有独立的牌组、生命与能量，轮流行动。

- **服务器中转（默认）**：联机中转服务内置于 Next.js（`/api/mp` 路由，HTTP 轮询收发），默认连接官方服务器 `https://slaythespire.space-z.ai`，自托管时自动回退到本机；大厅实时房间列表 + 4 位房间码
- **P2P 直连（可选）**：PeerJS + WebRTC 直连，无需服务器，房间码互通（无大厅列表，受网络环境影响）
- 房主权威架构：房主执行全部游戏逻辑并广播快照，客机动作经中转/直连转发给房主执行
